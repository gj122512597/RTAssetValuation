...
/**
 * 尽调（Due Diligence）单条检查项
 *  - 每类资产有标准化模板（写字楼 50 项 / 商铺 30 项 ...）
 *  - 客户经理现场勾选 / 填备注 / 拍照
 *  - 完成后系统自动填入 ai_features.physical.* （数据回流）
 */
export type CheckResult = 'pass' | 'warn' | 'fail' | 'na' | 'pending';

export interface DueDiligenceCheck {
  id: string;
  category: string;            // e.g. "基础信息", "物理状态", "周边配套", "法律合规", "政策环境"
  label: string;               // 具体检查项
  required: boolean;           // 是否必检（必检项未填则阻断后续流程）
  result: CheckResult;
  note?: string;
  /** 现场拍照占位（mock 用标识符） */
  photos?: string[];
...
export interface DueDiligenceProgress {
  ...
  /** 完成度 0-1 */
  completion: number;
  /** 总分（pass=1, warn=0.6, fail=0, na 跳过） */
  score: number;
  /** 必检项是否全部完成 */
...
/**
 * 资产接收 / 新建（待尽调资产）
 *  - 来自部队接收 / 新购入 / 拍卖竞得 / 政府划拨
 *  - 跑完尽调 → 进入资产池 → 业务流可继续
 */
export type IntakeSource = 'military_transfer' | 'purchase' | 'auction' | 'government_grant' | 'other';

export type IntakeStatus = 'todo' | 'in_progress' | 'completed' | 'rejected';

export interface IntakeAsset {
  id: string;                   // 内部 ID（与将来 Asset.id 一致）
  name: string;                  // 资产名（暂用）
  type: BusinessType | string;    // 业态
  region: string;
  address?: string;
  area?: number;                 // 初始估值（来自卖方/移交文件）
  initial_price?: number;        // 卖方/移交方报价
  source: IntakeSource;
  priority: 'low' | 'mid' | 'high';
  submitted_by: string;          // 提交人
  submitted_at: string;
  due_date?: string;             // 尽调截止
  /** 关联的尽调进度（如果已启动） */
  progress?: DueDiligenceProgress;
  status: IntakeStatus;
  /** 完成时关联的资产 ID（进入资产池） */
